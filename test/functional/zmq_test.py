...
from test_framework.mininode import dftzhash
...
def dftzhash_helper(b):
    return encode(dftzhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that dftz has been built with ZMQ enabled
        ...
        if not config["components"].getboolean("ENABLE_ZMQ"):
            raise SkipTest("dftzd has not been built with zmq enabled.")
