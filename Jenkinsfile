...
// which are part of the DFTz repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="dftz-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /dftz-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "dftzpay-dftz/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from dftzpay-dftz/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'dftzpay-dftz/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from dftzpay-dftz/develop"
            }
          }

          if (hasCache) {
            sh "cd /dftz-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /dftz-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /dftz-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /dftz-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /dftz-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /dftz-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /dftz-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
