...
*Setup instructions for a Gitian build of DFTz Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DFTz
...
- [Building DFTz Core](#building-dftz-core)
...
Clone the git repositories for DFTz Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/dftzpay/dftz
...
in the DFTz Core repository under 'Fetch and create inputs' to install sources which require
...
To build DFTz Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the DFTz Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dftz/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dftzpay/dftz
...
For example:
```bash
URL=https://github.com/crowning-/dftz.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dftz=${COMMIT} --url dftz=${URL} ../dftz/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dftz=${COMMIT} --url dftz=${URL} ../dftz/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dftz=${COMMIT} --url dftz=${URL} ../dftz/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the dftz git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../dftz/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/dftz
SIGPATH=/some/root/path/dftz-detached-sigs

./bin/gbuild --url dftz=${BTCPATH},signature=${SIGPATH} ../dftz/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dftz-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dftz-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dftz-osx-build.assert
```
...
[dftz/gitian.sigs](https://github.com/dftzpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
