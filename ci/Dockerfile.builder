...
# dftz_hash
RUN git clone https://github.com/dftzpay/dftz_hash
RUN cd dftz_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} dftz
RUN useradd -u ${USER_ID} -g dftz -s /bin/bash -m -d /dftz dftz
...
RUN mkdir /dftz-src && \
  ...
  chown $USER_ID:$GROUP_ID /dftz-src && \
